...
- 지원 도구:
  ...
  - ➤ 화살표 (시작점→끝점 방향)
  - T 텍스트 입력 (영역 지정 후 입력)
  - ↺/↻ 실행 취소·다시 실행 (그리기/영역 마스킹 통합 히스토리)
- ❌ 그리기 모드 종료
- 🎨 색상 및 선 굵기 커스터마이징 지원
- 📐 도구모음 접기/펴기 기능
...
2. 화면 왼쪽 상단에 나타나는 도구 모음에서 원하는 도구 선택:
   ...
   - **화살표**: 시작점에서 드래그 종료 지점을 향하는 화살표 그리기 (설정 색상 적용)
   - **텍스트**: 입력할 영역을 드래그 → 입력창에 텍스트 작성 후 Ctrl/⌘+Enter 또는 포커스 해제로 확정
...
- 단축키
  - Undo: `Ctrl + Z` / `⌘ + Z`
  - Redo: `Ctrl + Shift + Z` / `⌘ + Shift + Z` 또는 `Ctrl + Y`
- 통합 히스토리
  - 그리기와 영역 마스킹 작업을 하나의 타임라인으로 기록합니다.
  - 예) 그리기 → 영역 마스킹 → 그리기 → 영역 마스킹 후 Undo를 누르면
    - 영역 마스킹 취소 → 그리기 취소 → 영역 마스킹 취소 → 그리기 취소 순서로 되돌립니다.
- 텍스트 입력 중에는 브라우저의 기본 편집 Undo/Redo를 방해하지 않도록 처리됩니다. 텍스트 확정 후부터 전역 Undo/Redo에 포함됩니다.
...
- **그리기 설정**:
  ...
  - 텍스트 크기 (기본값: 16px)
  - 도구모음 초기 접힘 상태
  - 미리보기: 선 굵기/색상, 텍스트 크기 실시간 미리보기 제공
- **텍스트 마스킹 설정**:
  - 마스킹 문자 (기본값: *)
- **영역 마스킹 설정**:
  - 블러 강도 (기본값: 10px)
